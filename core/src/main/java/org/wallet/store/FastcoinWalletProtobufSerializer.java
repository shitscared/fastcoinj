...
import com.google.fastcoin.wallet.WalletTransaction;
 ...
 * format is defined by the <tt>fastcoin.proto</tt> file in the fastcoinj source distribution.<p>
 *
 ...
 */
public class FastcoinWalletProtobufSerializer extends WalletProtobufSerializer {
    private static final Logger log = LoggerFactory.getLogger(FastcoinWalletProtobufSerializer.class);

    // Early version of name-value value for use in protecting encrypted wallets from being loaded
    // into earlier versions of FastcoinWallet. Unfortunately I merged this into the FastcoinWallet v0.4 code by mistake.
    // @deprecated replaced by ORG_FASTCOIN_WALLET_WALLET_PROTECT_2
    public static final String ORG_FASTCOIN_WALLET_WALLET_PROTECT = "org.wallet.walletProtect";

    public static final String ORG_FASTCOIN_WALLET_WALLET_PROTECT_2 = "org.wallet.walletProtect.2";

    public FastcoinWalletProtobufSerializer() {
        super();
    }
        ...
        if (walletProto.hasVersion()) {
            ...
            if (version == 0) {
                if (walletEncryptionType == EncryptionType.ENCRYPTED_SCRYPT_AES) {
                    ...
                    wallet.setVersion(FastcoinWalletVersion.PROTOBUF_ENCRYPTED);
                } else if (walletEncryptionType == EncryptionType.UNENCRYPTED) {
                    // If it is unencrypted it is protobuf.2
                    wallet.setVersion(FastcoinWalletVersion.PROTOBUF);
                } else {
            ...
            } else if (version == FastcoinWalletVersion.PROTOBUF.getWalletVersionAsInt()) {
                wallet.setVersion(FastcoinWalletVersion.PROTOBUF);
            } else {
                if (version == FastcoinWalletVersion.PROTOBUF_ENCRYPTED.getWalletVersionAsInt()) {
                    wallet.setVersion(FastcoinWalletVersion.PROTOBUF_ENCRYPTED);
                } else {
        ...
        for (Protos.Extension extProto : walletProto.getExtensionList()) {
            ...
            if (extension == null) {
                if (extProto.getMandatory()) {
                    // If the extension is the ORG_FASTCOIN_WALLET_WALLET_PROTECT or ORG_FASTCOIN_WALLET_WALLET_PROTECT_2 then we know about that.
                    // This is a marker extension to prevent earlier versions of wallet loading encrypted wallets.
                    
                    // Unfortunately I merged the recognition of the ORG_FASTCOIN_WALLET_WALLET_PROTECT mandatory extension into the v0.4 code
                    // so it could load encrypted wallets mistakenly.
                    
                    // Hence the v0.5 code now writes ORG_FASTCOIN_WALLET_WALLET_PROTECT_2.
                    if (!(extProto.getId().equals(FastcoinWalletProtobufSerializer.ORG_FASTCOIN_WALLET_WALLET_PROTECT) ||
                            extProto.getId().equals(FastcoinWalletProtobufSerializer.ORG_FASTCOIN_WALLET_WALLET_PROTECT_2))) {
    ...
    protected void readTransaction(Protos.Transaction txProto, NetworkParameters params) throws UnreadableWalletException {
        ...
        // (This code is is here because some old FastcoinWallet serialised wallets had the same tx appearing twice and the wallets would not load).
    ...
    protected WalletTransaction connectTransactionOutputs(org.fastcoinj.wallet.Protos.Transaction txProto) throws UnreadableWalletException {
