...
public abstract class Message implements Serializable {
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = fastcoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the fastcoin wire protocol.
     * <br/>
    ...
    public byte[] unsafeFastcoinSerialize() {
        ...
        try {
            fastcoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to fastcoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the fastcoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void fastcoinSerialize(OutputStream stream) throws IOException {
