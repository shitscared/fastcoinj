 ...
 * the lightweight SPV mode, a {@link com.google.fastcoin.store.SPVBlockStore} is the right choice.</p>
 *
 ...
 * {@link com.google.fastcoin.store.H2FullPrunedBlockStore} implements full verification, which is equivalent to the
...
public abstract class AbstractBlockChain {
    ...
    private boolean add(Block block, boolean tryConnecting,
        ...
        try {
            ...
            // article here for more details: http://code.google.com/p/fastcoinj/wiki/SecurityModel
            try {
                block.verifyHeader();
        ...
        for (Transaction tx : transactions) {
            try {
                if (listener.isTransactionRelevant(tx)) {
                    ...
                    if (clone)
                        tx = new Transaction(tx.params, tx.fastcoinSerialize());
  ...
  //          timespan = params.targetTimespan / 32;
        ...
        // used for unit test only, to reverse the encoded big values of difficulty. remove in production, the below line is not needed
        //long newDiffLong = Utils.encodeCompactBits(newDifficulty);
    ...
    private void checkTestnetDifficulty(StoredBlock storedPrev, Block prev, Block next) throws VerificationException, BlockStoreException {
        ...
        // There is an integer underflow bug in fastcoin-qt that means mindiff blocks are accepted when time
        // goes backwards.
