...
import com.google.fastcoin.script.ScriptChunk;
...
import com.google.fastcoin.wallet.*;
import com.google.fastcoin.wallet.WalletTransaction.Pool;
...
import static com.google.fastcoin.core.Utils.fastcoinValueToPlainString;
...
//   for transactions independent of the transactions themselves, so the timeline can be walked without haing to
 ...
 *
 * <p>To learn more about this class, read <b><a href="http://code.google.com/p/fastcoinj/wiki/WorkingWithTheWallet">
 ...
 * other objects, see the <a href="http://code.google.com/p/fastcoinj/wiki/GettingStarted">Getting started</a> tutorial
 * on the website to learn more about how to set everything up.</p>
 *
 * <p>Wallets can be serialized using either Java serialization - this is not compatible across versions of fastcoinj,
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.fastcoin.wallet.WalletFiles.Listener)}
...
public class Wallet implements Serializable, BlockChainListener, PeerFilterProvider {
    ...
    private FastcoinWalletVersion version;
    ...
    public void receivePending(Transaction tx, @Nullable List<Transaction> dependencies, boolean overrideIsRelevant) throws VerificationException {
        ...
        try {
            ...
            if (log.isInfoEnabled()) {
                log.info(String.format("Received a pending transaction %s that spends %s FST from our own wallet," +
                        " and sends us %s FST", tx.getHashAsString(), Utils.fastcoinValueToFriendlyString(valueSentFromMe),
                        Utils.fastcoinValueToFriendlyString(valueSentToMe)));
            }
    ...
    private void receive(Transaction tx, StoredBlock block, BlockChain.NewBlockType blockType,
        ...
        log.info("Received tx{} for {} FST: {} [{}] in block {}", sideChain ? " on a side chain" : "",
                fastcoinValueToFriendlyString(valueDifference), tx.getHashAsString(), relativityOffset,
    ...
    public static class SendRequest {
         ...
         * (its value is ignored and set to {@link com.google.fastcoin.core.Wallet#getBalance()} - the fees required
        ...
        /**
         * If not null, the {@link com.google.fastcoin.wallet.CoinSelector} to use instead of the wallets default. Coin selectors are
     ...
     */
    public boolean completeTx(SendRequest req) throws InsufficientMoneyException {
        return completeTx(req, true);
        //Huck: check this to amke sure its still good
        /*lock.lock();
        try {
            ...
            // We need to know if we need to add an additional fee because one of our values are smaller than 0.01 FST
    ...
    public boolean completeTx(SendRequest req, boolean sign) {
        ...
        try {
            ...
            // Keep a track of the date the tx was created (used in FastcoinWalletService
    ...
    private boolean adjustOutputDownwardsForFee(Transaction tx, CoinSelection coinSelection, BigInteger baseFee, BigInteger feePerKb) {
        ...
        int size = tx.fastcoinSerialize().length;
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, com.google.fastcoin.wallet.WalletFiles.Listener)}
    ...
    public String toString(boolean includePrivateKeys, boolean includeTransactions, boolean includeExtensions,
        ...
        try {
            ...
            builder.append(String.format("Wallet containing %s FST in:%n",
                    fastcoinValueToPlainString(balance)));
    ...
    public void reorganize(StoredBlock splitPoint, List<StoredBlock> oldBlocks, List<StoredBlock> newBlocks) throws VerificationException {
        ...
        try {
            ...
            log.info("post-reorg balance is {}", Utils.fastcoinValueToFriendlyString(balance));
     ...
     * of {@link com.google.fastcoin.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
        ...
        try {
            ...
            for (ECKey key : keychain) {
                if (key.isEncrypted()) {
                    ...
                    // (all fastcoin controlled by that private key is lost forever).
     ...
     */
    public FastcoinWalletVersion getVersion() {
        return version;
    }
     ...
     */
    public void setVersion(FastcoinWalletVersion version) {
     ...
     * you can use {@link com.google.fastcoin.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    private class FeeCalculation {
        ...
        public FeeCalculation(SendRequest req, BigInteger value, List<TransactionInput> originalInputs,
            ...
            if (selection3 == null && selection2 == null && selection1 == null) {
                ...
                log.warn("Insufficient value in wallet for send: needed {} more", fastcoinValueToFriendlyString(valueMissing));
     ...
     */
    public void setTransactionBroadcaster(@Nullable com.google.fastcoin.core.TransactionBroadcaster broadcaster) {
    ...
    private Transaction rekeyOneBatch(long keyRotationTimestamp, final TransactionBroadcaster broadcaster) {
        ...
        try {
            ...
            checkState(rekeyTx.fastcoinSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            commitTx(rekeyTx);
