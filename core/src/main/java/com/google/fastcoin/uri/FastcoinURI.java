 ...
 *
 * <ul>
 * <li>{@code fastcoin:<address>}</li>
 * <li>{@code fastcoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 */
public class FastcoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(FastcoinURI.class);
    ...
    /**
     * Constructs a new FastcoinURI from the given string. Can be for any network.
     *
    ...
    public FastcoinURI(@Nullable NetworkParameters params, String input) throws FastcoinURIParseException {
        ...
        try {
            ...
            throw new FastcoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  fastcoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form fastcoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the fastcoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : fastcoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("fastcoin://")) {
            schemeSpecificPart = input.substring("fastcoin://".length());
        } else if (input.startsWith("fastcoin:")) {
            schemeSpecificPart = input.substring("fastcoin:".length());
        } else {
            throw new FastcoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0 || "".equals(addressSplitTokens[0])) {
            throw new FastcoinURIParseException("Missing address");
        }
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new FastcoinURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
    ...
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws FastcoinURIParseException {
        ...
        try {
            ...
            throw new FastcoinURIParseException("Bad address", e);
        }
        ...
        for (String nameValuePairToken : nameValuePairTokens) {
            ...
            if (tokens.length != 2 || "".equals(tokens[0])) {
                throw new FastcoinURIParseException("Malformed Bitcoin URI - cannot parse name value pair '" +
                        nameValuePairToken + "'");
            }
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("FastcoinURI[");
        boolean first = true;
    ...
    public static String convertToFastcoinURI(Address address, BigInteger amount, String label, String message) {
        return convertToFastcoinURI(address.toString(), amount, label, message);
    }
    ...
    public static String convertToFastcoinURI(String address, @Nullable BigInteger amount, @Nullable String label,
        ...
        if (amount != null) {
            ...
            builder.append(Utils.fastcoinValueToPlainString(amount));
