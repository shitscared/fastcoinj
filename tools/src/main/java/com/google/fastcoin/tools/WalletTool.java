    ...
    public static void main(String[] args) throws Exception {
        ...
        switch (netFlag.value(options)) {
            ...
            case TEST: 
                ...
                discovery = new IrcDiscovery("#fastcoinTEST3");
                break;
            default:
                throw new RuntimeException("Unreachable.");
        }
    ...
    private static void send(List<String> outputs, BigInteger fee, String lockTimeStr, boolean allowUnconfirmed) {
        try {
            ...
            if (!wallet.completeTx(req)) {
                System.err.println("Insufficient funds: have " + Utils.fastcoinValueToFriendlyString(wallet.getBalance()));
                return;
            }
            try {
                if (lockTimeStr != null) {
                ...
                wallet.addEventListener(new AbstractWalletEventListener() {
                    ...
                    public synchronized void onChange() {
                        ...
                        if (condition.matchLitecoins(balance)) {
                            System.out.println(Utils.fastcoinValueToFriendlyString(balance));
                            latch.countDown();
                        }
                    }
                });
